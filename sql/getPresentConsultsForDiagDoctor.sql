SELECT Event.id AS eventId,
    ...
    (SELECT datetime
        ...
        WHERE resTimestamp > 0 AND a2.deleted = 0 AND a5.deleted = 0 AND a12.deleted = 0 AND a5.event_id = Action.event_id AND a2.orgStructure_id = {$orgStructure_id} AND a5.createPerson_id != {$user_id} AND (a5.person_id = {$user_id} OR a1.status < 2 OR 'head'='diag_doctor') AND ((datetime >= '{$startDate}' AND datetime < '{$endDate}') OR (a1.status < 2 AND datetime < '{$todayDate}'))
        ORDER BY a1.status, datetime LIMIT 1
    ) AS `consult_datetime`,
    (SELECT a2.orgStructure_id
        FROM `Job_Ticket` as a1
        INNER JOIN Job as a2 ON (a1.master_id = a2.id)
        INNER JOIN ActionProperty_Job_Ticket as a33 ON (a33.value = a1.id)
        INNER JOIN ActionProperty as a4 ON (a33.id = a4.id)
    ...
    (SELECT a5.id
        ...
        AND a12.deleted =0 AND a5.event_id=Action.event_id AND a2.orgStructure_id={$orgStructure_id} AND a5.createPerson_id!={$user_id} AND (a5.person_id={$user_id} OR a1.status<2 OR 'head'='{$profile}') AND (( datetime >= '{$startDate}' AND datetime < '{$endDate}' ) OR ( a1.status < 2 AND datetime < '{$todayDate}' ) )
        ORDER BY a1.status, datetime LIMIT 1
		) AS `consult_id`,
    (SELECT a1.status
        FROM `Job_Ticket` as a1
...
HAVING (consult_class = {$orgStructure_id})
	AND (DATE(consult_datetime) >= DATE('{$startDate}') AND DATE(consult_datetime) < DATE('{$endDate}'))

ORDER BY OrgStructure_name, palata, lastName, firstName, patrName
